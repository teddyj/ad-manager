...
module.exports = async (req, res) => {
  try {
    ...
    // Catch-all error handler to ensure we always return a valid JSON response
    console.error('Unexpected error in fetch-product-image API:', outerError);
    return res.status(500).json({
      error: 'An unexpected error occurred while processing your request',
      alternativeSuggestion: 'Try uploading an image directly instead.',
      debug: process.env.NODE_ENV === 'development' ? outerError.message : undefined
    });
  }
}; 